...
[FOSS4G 2026 Workshop Timișoara, România](https://2026.europe.foss4g.org/)

![FOSS4G 2026 Workshop Timișoara, România](img/foss4g-europe-2026.png ) ![](img/postgresql_postgis.png)

![Workshop Program Link](https://talks.osgeo.org/foss4g-europe-2026-workshops/talk/J9RBVQ/)
...
This Workshop can use OSGeoLive (https://live.osgeo.org). OSGeoLive is based on Lubuntu and contains a collection of more than 50 pre-installed software projects. OSGeoLive also contains example data which will be used for the workshop.
...
* Natural Earth
  ...
  * https://live.osgeo.org/en/overview/naturalearth_overview.html
* OpenStreetMap
  * database: osm_local
...
PostGIS turned 25 on 21. May 2026!
...
```sql
SELECT ST_Letters('Good time at FOSS4G Europe 2026');
```
...
SELECT ST_SetSrid(
  ST_Translate(
    ST_Scale(
      ST_Letters('Good time at FOSS4G Europe 2026')
    , 0.00005, 0.00005)
    , 21.2034484, 45.7488206)
  ,4326);
```
...
1. Create your own database with the name **_foss4g_** use template database **_foss4g_template_**
...
3. Load / check for the extension **_postgis_** to your new database to be able to handle spatial data. (already done in the template database)
...
```sql
CREATE DATABASE foss4g TEMPLATE foss4g_template;
```

Move to database **_foss4g_** (update the object browser, go to the database foss4g and open a new query editor).


Check for the postgis extension. If it is not available you can create it via:
...
* create a point for Timișoara (Romania) with ST_MakePoint
* We take the coordinate of the UVT building which is latitude 45.7470571 longitude 21.2316306
...
INSERT INTO cities(
            name, geom, country)
    VALUES ('Timișoara',ST_SetSRID(ST_MakePoint(21.2316306, 45.7470571),4326),'Romania');
```
...
INSERT INTO cities(
            name, geom, country)
    VALUES ('Timișoara',ST_SetSRID(ST_MakePoint(17.796749 , 43.344108),4326),'Bosnia and Herzegovina');
```
...
```sql
SELECT ST_AsText(geom), geom FROM cities; -- with SRID
...
### Excercise 5: Create a table named locations via QGIS
...
### Excercise 6: Calculate the area for each country
...
### Excercise 8: Calculate the distance
...
* get back to your cities table. Calculate the distance between Timișoara and your home town.
...
* Question: Who had the longest distance to travel to Timișoara?
...
### Excercise 9:  Create a Spatial Index for your table cities
...
### Exercise 10: ST_Area calculations and RANK

Have a closer look at the provinces from Romania

* Create a view called qry_provinces_union
* Calculate the area of each province of Romania
* Order ist by area descendate
...
* use table ne_10m_admin_1_states_provinces and filter by admin Romania
...
Add a row number and display the geometry as text
...
Create a view. Assign the geometry type and projection to the new geom, add column admin (use GROUP BY)
...
### Excercise 13: ST_Distance and KNN

Have a look at the tables pois and buildings. 

Find out which bars/pubs are close to the Timișoara event location (see your table cities)

Use KNN (K nearest neighbor) to find the 5 closest pubs/bars from Timișoara Expo Building (see table cities)
...
Use KNN (K nearest neighbor) to find the pubs/bars less then 1000 m distance from Timișoara event location (see table cities)
...
SELECT p.osm_id, p.name, p.amenity, p.geom,
 ...
 pois p
  WHERE p.amenity IN ( 'bar' , 'pub')
   AND c.name = 'Timișoara'
   AND st_distance(c.geom, p.geom, true) < 1000
    ORDER BY
    c.geom <-> p.geom;
```

```sql
Select * from qry_next_bars_1000;
```

Create the 1000 m buffer around the Timișoara FOSS4G event location.

```sql
CREATE view qry_buffer_1000 as
 SELECT gid, st_buffer(geom::geography, 1000)::geometry as geom
  FROM cities 
   WHERE name = 'Timișoara';
```
...
SELECT b.osm_id, b.geom as geom,
       b.name,
       b.building,
       ST_Multi(ST_Intersection(b.geom, st_buffer(c.geom::geography, 1000)::geometry))::geometry(multipolygon,4326) as geom_intersection
FROM
  buildings b,
  cities c
WHERE
  ST_Intersects(b.geom, st_buffer(c.geom::geography, 1000)::geometry);
```
...
```sql
CREATE view qry_intersection_buffer_1000_buildings_2 as
SELECT b.osm_id, b.geom as geom,
       b.name,
       b.building,
       ST_Multi(ST_Intersection(b.geom, c.geom))::geometry(multipolygon,4326) as geom_intersection
FROM
  buildings b,
  qry_buffer_1000 c
WHERE
...
#. Grant read access to table ne_10m_admin_1_states_provinces to your new role workshop_reader
...
SELECT * from ne_10m_admin_1_states_provinces;
UPDATE ne_10m_admin_1_states_provinces SET name = 'TEST' WHERE name = 'Toscana';
--ERROR:  permission denied for relation ne_10m_admin_1_states_provinces
...
-- Run the following SQL
SELECT * from cities;
UPDATE cities SET name = 'TEST' WHERE name = 'Timișoara';
```
